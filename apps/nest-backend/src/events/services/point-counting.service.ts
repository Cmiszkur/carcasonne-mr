...
export class PointCountingService {
  ...
  public checkNewTile(
    ...
    const placedFallower: FollowerDetails | undefined =
      copiedPlacedTile.fallowerDetails;
    const players: Player[] = room.players;
    const cities: [Position[]] | undefined =
      copiedPlacedTile.tileValuesAfterRotation?.cities;
    const roads: [Position[]] | undefined =
      copiedPlacedTile.tileValuesAfterRotation?.roads;
  ...
  private checkNextTile(
    ...
    citiesOrRoads.forEach((positionSet) => {
      if (positionSet.length >= 2) {
        return this.checkAndMergeNearestPaths(
          positionSet,
          coordinates,
          pathData,
          tileId,
        ...
        const nextTile: ExtendedTile | null = this.extractNearestTile(
          board,
          coordinates,
          position
        );
  ...
  private searchForPathWithGivenCoordinates(
    ...
    return (
      Array.from(pathDataMap).find((array: [string, PathData]) => {
        return Array.from(array[1].countedTiles.values()).find(
          (countedTile) => {
            return this.tilesService.checkCoordinates(
              countedTile.coordinates,
              coordinates
            );
          }
        );
  ...
  private countPoints(
    ...
    console.log(
      'basePoints',
      basePoints,
      searchedPathData.points,
      extraPoints && isCities ? basePoints * 2 : basePoints,
      extraPoints
    );
    searchedPathData.points +=
      extraPoints && isCities ? basePoints * 2 : basePoints;
  ...
  private updatePathOwners(
    ...
    if (searchedPathData && fallowerOwner) {
      if (
        searchedPathData.pathOwners.some(
          (pathOwner) => pathOwner !== fallowerOwner
        )
      )
        return;
  ...
  private filterCompletedPathDataMap(pathDataMap: PathDataMap): PathDataMap {
    return new Map(
      Array.from(pathDataMap).filter(([key, value]) => !value.completed)
    );
  }

  private getPathId(
    pathDataMap: PathDataMap,
    nextTile: ExtendedTile | null
  ): string | undefined {
    if (nextTile) {
  ...
  private checkPathCompletion(
    ...
    newOrUpdatedPathIds.forEach((newOrUpdatedPathId) => {
      const checkedPath = pathDataMap.get(newOrUpdatedPathId);
      if (checkedPath) {
        checkedPath.completed = this.checkAllTilesComplition(
          board,
          checkedPath
        );
      }
  ...
  private checkAllTilesComplition(
    board: ExtendedTile[],
    checkedPath: PathData
  ): boolean {
    const isCompleted: boolean[] = [];
  ...
  private logPaths(
    roadsPathDataMap: PathDataMap,
    citiesPathDataMap: PathDataMap
  ): void {
    roadsPathDataMap.forEach((pathData, pathId) => {
      ...
      pathData.countedTiles.forEach((countedTile, tileId) => {
        ...
        console.log(
          '   tile checked positions: ',
          countedTile.checkedPositions
        );
        console.log('   =============================================');
      });
      console.log('==================================================');
